...
abstract class Builder implements \ArrayAccess
{
    ...
    protected function checkController()
    {
        ...
        if (!$class_exists) {
            ...
            if (!\Team\Loader\Classes::load($this->controller, $class_file, $this->base)) {
                return \Team::system("Controller class file {$class_file} not found",
                    '\team\responses\Response_Not_Found');
            }
        }
        ...
        if (!$this->checkParent($this->controller)) {
            return \Team::system("Controller class {$this->controller} hasn't got a good parent. Check it",
                '\team\responses\Response_Not_Found');
        }

        if (!$is_a_controller) {
            return \Team::system("Controller class {$this->controller} isn't a \\team\\responses\\controller. Check namespaces and names",
                '\team\responses\Response_Not_Found');
        }
    ...
    protected function setComponent($component)
    {
        ...
        if (empty($component)) {
            \Team::system("Component in app '{$this->app}' not specified", '\team\responses\Response_Not_Found',
                $this->get(), $level = 5);
        }
    ...
    protected function setResponse($response)
    {
        ...
        $response = (empty($response) || ("_" == $response[0])) ? \Team\Data\Filter::apply('\team\default_response',
    ...
    protected function getMethod($class, $response, $reflection_class, $is_default = false)
    {
        ...
        if ($response_exists) {
            ...
            if ($response_exists) {
                if ($this->app == \Team\System\Context::before('APP') && $method->isProtected()) {
                    ...
                    if (!is_callable([$class, $response]) || !$method->isPublic()) {
        ...
        if (!$response_exists) {
            return \Team::system("Response method '{$this->response}' in Controller class $class  not found",
                '\team\responses\Response_Not_Found', $this->get(), $level = 5);
        }
