...
/** Activamos los errores. Ninguno: error_reporting(0) */
final  class Debug
{

    private function __construct()
    { /* prohibida instanciar una clase */
    }
    ...
    */
    ...
    private static function withData($var, $label, $file, $line)
    {
        ...
        if ($is_object || $is_array) {
            $data->setContext('VIEW',
                \Team\Config::get('\team\Debug\compound_template', 'team:framework/debug/compound'));
            $data->label = $label;
            $data->sublabel = null;
    ...
    public static function me($var = 'Hello, World', $label = false, $file = null, $line = null, $level = null)
    {
        ...
        if (!\team\Config::get('SHOW_ERRORS', false)) {
            return;
        }

        if (false === \Team\Config::get('SHOW_IN_NAVIGATOR', false) || \Team\System\Context::main("AJAX")) {
    ...
    public static function time($_op = "start", $_display = 0)
    {
        ...
        switch ($_op) {
            ...
            case "stop":
                if ($start) {
                    $time = ($start) ? microtime(true) - $start : 0;
                }
                $start = 0;
                break;
